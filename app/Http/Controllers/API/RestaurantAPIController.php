...
use App\Criteria\Restaurants\NearRestaurantCriteria;
...
class RestaurantAPIController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
        ...
        if (!empty($this->restaurantRepository)) {
            try {
                ...
                if ($request->has(['myLon', 'myLat'])) {
                    $this->restaurantRepository->pushCriteria(new NearRestaurantCriteria($request));
                }
            } catch (RepositoryException $e) {
                return $this->sendError($e->getMessage());
            }
            ...
            $restaurant = Helper::isThisStoreOpen($restaurant);
        ...
        if (empty($restaurant)) {
            ...
            return $this->sendResponse($data,'Restaurant not found');
