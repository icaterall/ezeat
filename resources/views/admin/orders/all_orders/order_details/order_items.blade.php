                ...
                <th class="text-right pr-0 font-weight-bold text-muted text-uppercase">Price</th>
                  ...
                  @foreach($order->foodOrders as $foods)
                    <tr class="font-weight-boldest font-size-lg">
                       <td class="pl-0 pt-7">
                            {{ $foods->food->name }} 
                               @ MYR {{number_format($foods->food_price,2)}}
                    ...
                    <div class="font-weight-bold order_extra">
                                ...
                                <?php $count = 0; ?>
                                   @php 
                                    ...
                                    $arr = []; 
                                        ...
                                        {$arr[] = $extra->extras->name.'-- MYR '.number_format($extra->price,2);
                                    }else 
                        ...
                        @endif
                               ...
                               <td class="text-danger pr-0 pt-5 text-center">
                                                {{ $foods->quantity }}</td> 
                                            <td class="text-danger pr-0 pt-7 text-right">MYR {{number_format($foods->price,2)}}</td>
                                          </tr>
                            ...
                            <div class="d-flex justify-content-between flex-column flex-md-row font-size-lg">
                                <div class="d-flex flex-column mb-10 mb-md-0">
                                    <div class="d-flex justify-content-between mb-3">
                                        ...
                                        <span class="text-right">MYR {{number_format($order->subtotal,2)}}</span>
                                    ...
                                    @if($order->isdelivery == 1)
                                        <div class="d-flex justify-content-between mb-3">
                                            ...
                                            <span class="text-right">MYR {{number_format($order->delivery_fee,2)}}</span>
                                        </div>
                                    @endif
                                ...
                                @if($order->tax > 0)
                                  <div class="d-flex justify-content-between">
                                   ...
                                   <span class="text-right">MYR {{ number_format($order->tax,2)}}</span>
                                 </div>
                                @endif

                                @if($order->service_charge>0)
                                    <div class="d-flex justify-content-between">
                                        <span class="mr-15 font-weight-bold">Service Charge</span>
                                        <span class="text-right" style="color: orange">MYR {{number_format($order->service_charge,2)}}</span>
                                    </div>
                                @endif 
                                  ...
                                  @if($order->discount!=0)
                                    <div class="d-flex justify-content-between">
                                     <span class="mr-15 font-weight-bold special_inst"  style="color: green">Discount</span>
                                        <span class="text-right" style="color: green">MYR -{{number_format($order->discount,2)}}</span>
                                  </div>
                                    @endif
                                  
                                    @if($order->tips > 0)
                                    <div class="d-flex justify-content-between">
                                        <span class="mr-15 font-weight-bold">{{ ($order->isdelivery =='1')?"Rider Tip":"Restaurant Tip" }}</span>
                                ...
                                <div class="d-flex flex-column text-md-right">
                                  ...
                                  <span class="font-size-h2 font-weight-boldest text-danger mb-1">MYR {{number_format($order->total,2)}}</span>
                                </div>
                            </div>
                        </div>
                    </div>
